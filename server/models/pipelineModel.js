...
const Pipeline = {
  ...
  createEstimate: async (
    ...
    try {
      await client.query("BEGIN"); // Start transaction lane
  ...
  //CONVERT TO SALES ORDER (Reserve Stock)
  convertToSalesOrder: async (job_id) => {
    ...
    try {
      ...
      for (const item of items.rows) {
        ...
        // Trigger rollback immediately if stock is insufficient
        if (res.rowCount === 0) {
          throw new Error(
            `Insufficient stock for inventory ID ${item.inventory_id}. Transaction aborted.`,
          );
        }
      }
  ...
  // CONVERT TO INVOICE (Permanent Deduction)
  ...
  //CANCEL JOB (Release Reserved Stock)
  ...
  //KANBAN DATA & STATUS UPDATES
  getBranchJobs: async (branch_id, limit, offset) => {
    const countResult = await pool.query(
      `SELECT COUNT(*) FROM service_jobs sj WHERE sj.branch_id = $1 AND sj.status != 'Cancelled'`,
      [branch_id],
    );
    ...
    return { data: result.rows, totalRecords: totalRecords };
  },

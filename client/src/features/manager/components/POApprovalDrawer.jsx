      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          <motion.div
            ...
            className={`absolute inset-0 bg-slate-900/40 backdrop-blur-sm ${!isSubmitting ? "cursor-pointer" : "cursor-wait"}`}
            aria-hidden="true"
          />
          ...
          <motion.div
            ...
            className="relative w-full sm:w-[500px] md:w-[600px] bg-slate-50 dark:bg-slate-900/95 shadow-2xl flex flex-col border-l border-slate-200 dark:border-slate-800"
            role="dialog"
            aria-modal="true"
          >
            {/* Header */}
            <header className="flex justify-between items-start px-6 py-5 sm:px-8 sm:py-6 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0 z-10 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)]">
              ...
              <button
                ...
                className="p-2.5 -mr-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all active:scale-95 cursor-pointer shrink-0 disabled:opacity-50 disabled:cursor-wait"
                ...
                <div className="space-y-6 sm:space-y-8">
                  ...
                  {po.notes && (
                    <div className="p-5 sm:p-6 bg-amber-50 dark:bg-amber-500/5 border border-amber-200 dark:border-amber-500/20 rounded-[20px] sm:rounded-[24px]">
                      ...
                      <p className="text-xs sm:text-sm text-amber-900 dark:text-amber-200/80 italic leading-relaxed whitespace-pre-wrap break-words">
                        "{po.notes}"
                      </p>
                    </div>
                  )}
                  ...
                  {po.status === "PENDING_APPROVAL" && (
                    <div className="p-5 sm:p-6 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-[20px] sm:rounded-[24px] space-y-4">
                      ...
                      <textarea
                        ...
                        rows="3"
                        disabled={isSubmitting}
                        className="w-full px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-xl text-xs font-medium text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 resize-none disabled:opacity-50 shadow-sm transition-all"
                      />
                    </div>
                  )}
                </div>
            ...
            {po?.status === "PENDING_APPROVAL" && !loading && (
              <div className="p-4 sm:p-6 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-3 bg-white dark:bg-slate-900 shrink-0">
                <button
                  type="button"
                  onClick={() => handleDecision("REJECTED")}
                  disabled={isSubmitting}
                  className="flex-1 py-3.5 sm:py-4 bg-white dark:bg-slate-800 border border-rose-200 dark:border-rose-500/20 hover:bg-rose-50 dark:hover:bg-rose-500/10 text-rose-600 dark:text-rose-400 font-black rounded-xl text-[10px] sm:text-xs uppercase tracking-widest transition-all active:scale-[0.98] flex justify-center items-center gap-2 disabled:opacity-50 shadow-sm"
                >
                  {isSubmitting ? (
                ...
                <button
                  ...
                  disabled={isSubmitting}
                  className="flex-[2] py-3.5 sm:py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl text-[10px] sm:text-xs uppercase tracking-widest transition-all active:scale-[0.98] flex justify-center items-center gap-2 shadow-lg shadow-emerald-500/20 disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (

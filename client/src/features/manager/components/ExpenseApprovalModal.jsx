...
const getConfidenceColor = (score) => {
  if (!score)
    return "border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-900/50"; // No AI data
  if (score < 0.7)
    return "border-red-400 bg-red-50 dark:bg-red-500/10 ring-2 ring-red-400 ring-opacity-50"; // CRITICAL: Low confidence
  if (score < 0.85)
    return "border-amber-400 bg-amber-50 dark:bg-amber-500/10 ring-2 ring-amber-400 ring-opacity-50"; // WARNING: Medium confidence
  return "border-emerald-400 bg-emerald-50 dark:bg-emerald-500/10"; // SUCCESS: High confidence
};
  ...
  useEffect(() => {
    if (isOpen && expense) {
      setFormData({
        ...
        expense_account_id: 5000, // Default to COGS for capstone example
        items: expense.items || [],
      });

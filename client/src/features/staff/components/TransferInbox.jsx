...
const TransferInbox = ({ transfers, onReceive }) => {
  return (
    <div className="space-y-4">
      <h3 className="text-[10px] font-black uppercase text-slate-400 tracking-widest px-2">Incoming Transfers (In Transit)</h3>
      ...
      {transfers.length === 0 ? (
        <div className="text-center py-12 bg-slate-50 dark:bg-black/10 rounded-3xl border border-dashed border-slate-200 dark:border-white/5">
          ...
          <p className="text-sm text-slate-400">No incoming deliveries at the moment.</p>
        </div>
      ) : (
        transfers.map((item) => (
          <div key={item.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-sm">
            <div className="flex gap-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900 dark:text-white">{item.partName}</h4>
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <p className="text-xs font-black text-blue-500 uppercase tracking-tighter">Qty to Receive</p>
                <p className="text-2xl font-black dark:text-white">{item.qty}</p>
              </div>
              <button 

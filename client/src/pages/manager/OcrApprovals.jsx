...
const OcrApprovals = () => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const hasInflation =
        result?.inflationDetected || result?.data?.inflationDetected;
      ...
      setTimeout(() => {
        ...
        if (hasInflation) {
          // Add a tiny delay to ensure the view switches smoothly before modal pops
          setTimeout(() => {
            setInflationAlert({
              show: true,
              message:
  ...
  return (
    <>
        ...
        <div className="space-y-4 animate-in slide-in-from-bottom-8 duration-500 pb-10">
          ...
          {actionStatus === "success" && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0 }}
              className="fixed top-10 left-1/2 -translate-x-1/2 z-[100] bg-emerald-500 text-white px-8 py-5 rounded-[24px] shadow-2xl flex items-center gap-5 font-bold border border-emerald-400"
            >
              ...
              <div className="w-full lg:w-7/12 bg-white dark:bg-slate-800 rounded-[32px] border border-slate-200 dark:border-white/10 p-6 sm:p-8 overflow-y-auto shadow-xl custom-scrollbar flex flex-col gap-6">
                ...
                {isTotalMismatched && (
                  <div className="p-3 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-xl flex items-center gap-3 text-red-600 dark:text-red-400 text-[10px] font-black uppercase tracking-widest shrink-0">
                    <AlertTriangle size={16} /> Total Mismatch: Line items sum
                    does not equal the Total Amount.
                  </div>
                )}
      ...
      {/* ==========================================
          GLOBAL MODALS (Rendered outside the view toggle)
          ========================================== */}
